...
const { pool, executeQuery } = require('../config/queueDatabase');
...
function resolvePosBranchId(user, req) {
  if (user.role === 'CASHIER' && user.branchId) return user.branchId;
  const simulated = req.headers['x-simulate-scope-type'];
  const simulatedId = req.headers['x-simulate-scope-id'];
  if (simulated === 'BRANCH' && simulatedId) return parseInt(simulatedId, 10);
  if (req.query.posBranchId) return parseInt(req.query.posBranchId, 10);
  return user.branchId || null;
}

// Resolve QMS branch from logged-in POS user
router.get('/resolve-branch', async (req, res) => {
  try {
    const posBranchId = resolvePosBranchId(req.user, req);
...
router.patch('/branches/:id/link-pos', async (req, res) => {
  try {
    if (req.user.role !== 'ADMIN') {
      return res.status(403).json({ success: false, message: 'Admin only' });
    }
    const { pos_branch_id } = req.body;
    await executeQuery('UPDATE qms_branches SET pos_branch_id = ? WHERE id = ?', [
      pos_branch_id || null,
      req.params.id,
    ]);
...
router.post('/public/:orgSlug/:branchSlug/call-next', async (req, res) => {
  ...
  try {
    ...
    let query = `SELECT t.id FROM qms_tickets t
  ...
  try {
    ...
    const stats = await executeQuery(
      `SELECT COUNT(*) AS total_today,
       ...
       FROM qms_tickets WHERE branch_id = ? AND date_key = ?`,
    ...
    const byService = await executeQuery(
      `SELECT s.name, s.prefix, s.color, COUNT(t.id) AS count
       ...
       WHERE s.branch_id = ? GROUP BY s.id ORDER BY s.display_order`,
